  ...
  Widget percentIndicator() {
    return FutureBuilder(
        ...
        builder: (BuildContext context, AsyncSnapshot<String> snapshot) {
          return FutureBuilder(
              future: Firestore.getCollection('Posts')
                  .where('userId', isEqualTo: snapshot.data)
                  .where('tagID', isEqualTo: 'Dog')
                  .get(),
              builder:
                  (BuildContext context, AsyncSnapshot<QuerySnapshot> percent) {
                if (!percent.hasData) {
                  this.progress = 0;
                  return Container(
                    child: Column(
                  ...
                  return Container(
                    child: Column(
                      children: [
                        ...
                        Text(percent.data.docs.length.toString() + " out of 5"),
                      ],
                    ),
                  );
                }
